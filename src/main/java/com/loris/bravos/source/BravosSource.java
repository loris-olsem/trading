...
public final class BravosSource {
  ...
  public record Scan(
      ...
      List<String> pages,
      boolean complete,
      Map<String, java.math.BigDecimal> dashboard) {}

  public BravosSource(Transport transport) {
  ...
  public void login(String username, String password) throws IOException {
    ...
    String action =
        form.attr("action").isBlank() ? "/my-account/" : safePath(form.absUrl("action"));
    Map<String, String> fields = new LinkedHashMap<>();
    for (Element e : form.select("input[type=hidden][name],button[name]"))
    ...
    fields.put(user.attr("name"), username);
    fields.put(pass.attr("name"), password);
    String encoded =
    ...
    var response =
        transport.request(
            "POST", action, Map.of("Content-Type", "application/x-www-form-urlencoded"), encoded);
    if (response.status() >= 300 && response.status() < 400)
      get(
          safePath(
              URI.create(ORIGIN + action)
                  .resolve(response.headers().getOrDefault("location", "/my-account/"))
                  .toString()));
    else if (response.status() != 200) throw new IOException("BRAVOS_LOGIN_FAILED");
    Document check = html(get("/my-account/"), "/my-account/");
    if (check.select("a[href*=customer-logout]").isEmpty())
      throw new IOException("BRAVOS_LOGIN_NOT_CONFIRMED");
  }
  ...
  public Scan scan(LocalDate floor, Collection<String> rereadUrls) throws IOException {
    for (int retry = 0; retry < 2; retry++) {
      ...
      for (int p = 1; p <= 500; p++) {
        ...
        Document page = html(get(path), path);
        pages.add(ORIGIN + path);
        List<Entry> found = archive(page);
        if (found.isEmpty()) {
          errors.add("ARCHIVE_EMPTY_OR_CHANGED:" + p);
          break;
        }
        String signature = found.stream().map(Entry::url).reduce("", (a, b) -> a + "|" + b);
        if (p == 1) first = signature;
        for (Entry entry : found)
          if (!entry.date().isBefore(floor)) entries.put(entry.url(), entry.url());
        if (found.stream().allMatch(e -> e.date().isBefore(floor))) {
          boundary = true;
          break;
        }
      }
      String recheck =
          archive(html(get(ARCHIVE), ARCHIVE)).stream()
              .map(Entry::url)
              .reduce("", (a, b) -> a + "|" + b);
      if (first != null && !Objects.equals(first, recheck)) {
        if (retry == 0) continue;
        errors.add("ARCHIVE_CHANGED_TWICE");
        boundary = false;
      }
      ...
      for (String u : entries.values()) {
        try {
          Article a = article(html(get(safePath(u)), safePath(u)), u);
          articles.add(a);
          alerts.add(parser.parse(a.key(), a.url(), a.date(), a.title(), a.body()));
        } catch (IllegalArgumentException | IOException e) {
          errors.add("ARTICLE_REQUIRES_REVIEW:" + u);
        }
      }
      Map<String, java.math.BigDecimal> dashboard =
          dashboard(html(get("/research/"), "/research/"));
  ...
  public List<Entry> archive(Document doc) {
    ...
    for (Element card : doc.select(".right")) {
      Element link =
          card.select("a[href]").stream()
              .filter(a -> a.text().length() > 15)
              .findFirst()
              .orElse(null);
      Matcher date = Pattern.compile("\\b(\\d{2}/\\d{2}/\\d{4})\\b").matcher(card.text());
      if (link != null && date.find())
        result.add(new Entry(ORIGIN + safePath(link.absUrl("href")), parseDate(date.group(1))));
    }
  ...
  public Article article(Document doc, String url) {
    ...
    if (!date.find() || title == null)
      throw new IllegalArgumentException("ARTICLE_METADATA_MISSING");
    ...
    return new Article(
        "bravos:" + article.id().replace("post-", "post:"), url, published, title.text(), body);
  }

  public Map<String, java.math.BigDecimal> dashboard(Document doc) {
    Map<String, java.math.BigDecimal> result = new LinkedHashMap<>();
    Element section =
        doc.select(".dash-newsletter").stream()
            .filter(e -> e.select(".h4").text().startsWith("Tactical Portfolio"))
            .findFirst()
            .orElse(null);
    if (section == null) return result;
    for (Element row : section.select(".asset-ratings .tbody .tr")) {
      var cells = row.select(".td");
      if (cells.size() != 3 || !cells.get(1).text().equals("Long"))
        throw new IllegalArgumentException("DASHBOARD_FORMAT_OR_DIRECTION_CHANGED");
      Matcher symbol =
          Pattern.compile("\\(\\$([A-Z][A-Z0-9.]{0,12})\\)").matcher(cells.getFirst().text());
      ...
      if (!weight.matches("\\d+(?:\\.\\d+)?") || result.containsKey(ticker))
        throw new IllegalArgumentException("DASHBOARD_WEIGHT_INVALID");
  ...
  private String get(String path) throws IOException {
    for (int redirects = 0; redirects <= 3; redirects++) {
      ...
      if (!Set.of(301, 302, 303, 307, 308).contains(response.status()))
        throw new IOException("BRAVOS_READ_FAILED");
      path =
          safePath(
              URI.create(ORIGIN + path)
                  .resolve(response.headers().getOrDefault("location", ""))
                  .toString());
    }
    throw new IOException("BRAVOS_REDIRECT_LIMIT");
  }

  private static Document html(String body, String path) {
    return Jsoup.parse(body, ORIGIN + path);
  }
  ...
  public static String safePath(String address) {
    ...
    if (!"https".equals(uri.getScheme())
        || !"bravosresearch.com".equals(uri.getHost())
        || uri.getPort() != -1
        || uri.getUserInfo() != null) throw new IllegalArgumentException("SOURCE_ORIGIN_ESCAPE");
