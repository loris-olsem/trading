...
public final class AlertParser {
    ...
    public Alert parse(String key, String url, LocalDate date, String title, String authored) {
        String body=authored.replace('\u00a0',' ').trim();
        ...
        BigDecimal price=optional("(?i)\\bat (?:a price of )?\\$"+N,first);
        ...
        if(action==Action.OPEN) {
            ...
            Matcher weights=Pattern.compile("(?i)from\\s+(?:a\\s+)?(?:weight(?:\\s+allocation)?(?:\\s+of)?\\s+)?"+N+"\\s+to\\s+(?:a\\s+)?(?:weight(?:\\s+of)?\\s+)?"+N).matcher(first);
            if(weights.find()) {
        ...
        if(action!=Action.OPEN) {
            stop=optional("(?i)(?:raising|moving|adjusting|lowering) our stop(?: loss)? from \\$[0-9.,]+ to \\$"+N,text);
        ...
        if(action!=Action.OPEN && Pattern.compile("(?i)(?:raising|moving|adjusting|lowering).{0,20}stop").matcher(text).find() && stop==null) fail("UNPARSED_STOP_CHANGE");
    ...
    private static BigDecimal number(String s) { return new BigDecimal(s.replace(',','.')); }
