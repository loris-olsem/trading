...
public final class HttpTransport implements Transport {
  ...
  public HttpTransport(URI origin, boolean cookies) {
    ...
    boolean official =
        "https".equals(origin.getScheme())
            && Set.of("public-api.etoro.com", "bravosresearch.com").contains(host)
            && origin.getPort() == -1;
    ...
    if ((!official && !loopback)
        ...
        || !List.of("", "/").contains(origin.getPath()))
      throw new IllegalArgumentException("UNAPPROVED_ORIGIN");
    this.origin = origin;
    var builder =
  ...
  public Response request(String method, String path, Map<String, String> headers, String body)
    ...
    if (!path.startsWith("/") || path.startsWith("//") || path.contains("\\"))
      throw new IOException("INVALID_REQUEST_PATH");
    URI uri = origin.resolve(path);
    if (!Objects.equals(uri.getHost(), origin.getHost())
        || uri.getPort() != origin.getPort()
        || !uri.getScheme().equals(origin.getScheme())
        || uri.getUserInfo() != null) throw new IOException("ORIGIN_ESCAPE");
    try {
      ...
      var response =
          client.send(
              ...
              HttpResponse.BodyHandlers.limiting(
                  HttpResponse.BodyHandlers.ofByteArray(), 4_000_000));
      byte[] bytes = response.body();
      Map<String, String> responseHeaders = new HashMap<>();
      for (String name : List.of("location", "retry-after", "content-type"))
        response.headers().firstValue(name).ifPresent(v -> responseHeaders.put(name, v));
      return new Response(
          response.statusCode(),
      ...
      throw new IOException("REQUEST_INTERRUPTED");
    } catch (IllegalArgumentException e) {
      throw new IOException("INVALID_HTTP_REQUEST");
    }
  }
}
