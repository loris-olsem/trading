...
public final class Configuration {
    ...
    public void validate() throws IOException {
        if(copySizingEvidence==null || copyStopsEvidence==null || assets==null) throw new IOException("INVALID_CONFIGURATION");
        Set<Long> ids=new HashSet<>();
        for(var e:assets.entrySet()) {
            Asset a=e.getValue();
            if(!e.getKey().matches("[A-Z][A-Z0-9.]{0,12}") || a==null || a.instrumentId<=0 || !ids.add(a.instrumentId)
                    || a.brokerSymbol==null || !a.brokerSymbol.matches("[A-Z][A-Z0-9.]{0,20}")
                    || a.unleveragedEvidence==null || a.unleveragedEvidence.isBlank()
                    || !Set.of("real","cfd").contains(a.settlementType) || a.priceScale<0 || a.priceScale>8 || a.unitScale<0 || a.unitScale>12)
                throw new IOException("INVALID_ASSET_CONFIGURATION");
        }
    }
}
